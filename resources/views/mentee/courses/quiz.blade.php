...
<div class="bg-primary pb-lg-64pt py-32pt">
    <div class="container page__container">
        <h2>Quiz</h2>
        <form method="POST" action="{{route("mentee.courses.quiz",[$course->id,md5($course->title),$section_id,$lecture_id])}}">
            @csrf
            <ul class="list-group">
                @foreach ($course->content->{array_keys($content)[$section_id]}->content[$lecture_id]->data as $qkey => $quiz)
                    <li class="list-group-item">Q{{$qkey+1}}. {{$quiz->question_title}}</li>
                        <ul class="list-group">
                            @foreach ($quiz->answers as $akey => $answer)
                            <div class="input-group mb-1">
                                <div class="input-group-prepend">
                                <div class="input-group-text">
                                    <input type="checkbox" name="answer{{$akey+1}}[]" id="{{$akey+1}}">
                                </div>
                                </div>
                                <input value="{{$answer->answer}}" class="form-control" disabled/>
                            </div>
                            @endforeach
                        </ul>
                @endforeach
            </ul>
            <button class="mt-3 btn btn-info">Submit</button>
        </form>
