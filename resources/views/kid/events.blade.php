                    ...
                    <tbody>
                        @foreach ($events as $event)
                            <tr>
                                ...
                                @if($event->event->mode=="Online")@if((new DateTime(Auth::user()->enrolled_programs->where("program_id",$program->program->id)->first()->date." ".Auth::user()->enrolled_programs->where("program_id",$program->program->id)->first()->time))<=(new DateTime("NOW"))->modify('+15 minutes'))<td><a href="{{$event->event->link}}" class="btn btn-info btn-sm">Join Now</a></td>@endif @endif
                            </tr>
                        @endforeach
                    </tbody>
                </table>
