...
class MenteeController extends Controller
{
    public function index(){
        $mentees = User::where(["inst_id"=>Auth::user()->id,"role"=>"Mentee"])->latest()->paginate(15);
        return view("organisation.mentee.index")->with([
            "mentees"=>$mentees
        ]);
    }
    ...
    public function delete(Request $request){
        ...
        if($user===NULL){
            ...
            if($user->inst_id!==Auth::user()->id){
                $request->session()->flash('error', "The mentee is not associated with your organisation.");
                return redirect()->back();
            }
            else{
                $user->inst_id = NULL;
                $user->save();
                $request->session()->flash('success', "The mentee is removed from your organisation");
                return redirect()->back();
            }
        }
    }
